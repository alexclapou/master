class SearchController < ApplicationController
  ...
  def search_stories
    ...
    @stories = Story.published.joins(:rich_text_content)
                    .where("action_text_rich_texts.body ILIKE ?", "%#{@search_term}%").paginate(page: params[:stories_page], per_page: 10)
    # .or(Story.published.where("title ilike ? ",
    #                           "%#{@search_term}%")).paginate(page: params[:stories_page], per_page: 9)
